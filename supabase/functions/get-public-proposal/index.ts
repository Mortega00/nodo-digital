...
Deno.serve(async (request: Request) => {
    ...
    try {
        ...
        const { data, error } = await serverClient.rpc("edge_get_public_proposal", {
            p_public_token: token
        });

        if (error || !isRecord(data)) {
            console.error("get-public-proposal RPC failed");
            return json({ ok: false, error: "proposal_lookup_failed" }, 500, origin);
        }
        ...
        console.error("get-public-proposal unexpected failure");
